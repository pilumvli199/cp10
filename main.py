#!/usr/bin/env python3
"""
main_patched.py - Full patched Advanced 720-Candles Crypto Trading Bot
- Relaxed/testable thresholds + SIGNAL_STRENGTH_TRIGGER env override
- Improved strength weighting & base confidence
- Conservative AI boost logic
- TEST_MODE for quick checks (no live saves/alerts when enabled)
- Detailed debug output for easier tuning
...
# final confidence threshold (lowered default for better signal flow; override via env)
SIGNAL_CONF_THRESHOLD = float(os.getenv("SIGNAL_CONF_THRESHOLD", "40.0"))  # final conf % (lowered for more signals)
# Allow env override for signal strength trigger
SIGNAL_STRENGTH_TRIGGER = int(os.getenv("SIGNAL_STRENGTH_TRIGGER", "2"))  # lowered trigger (more sensitive)
...
MIN_RR_RATIO = float(os.getenv("MIN_RR_RATIO", "1.0"))  # lowered RR requirement to allow more setups
TREND_REVERSAL_THRESHOLD = float(os.getenv("TREND_REVERSAL_THRESHOLD", "0.02"))  # 2%
TARGET_PROGRESS_LEVELS = [0.25, 0.50, 0.75, 0.90]

# TEST MODE: when true, do not save signals to Redis or send Telegram alerts
TEST_MODE = os.getenv("TEST_MODE", "false").lower() in ("1", "true", "yes")
if TEST_MODE:
    print("⚠️ Running in TEST_MODE: thresholds relaxed, no persistent saves or live alerts.")
    # in test mode optionally relax thresholds even further
    SIGNAL_CONF_THRESHOLD = float(os.getenv("TEST_SIGNAL_CONF_THRESHOLD", SIGNAL_CONF_THRESHOLD))
...
try:
    ...
    # Only ping when not in TEST_MODE, else keep it optional
    if not TEST_MODE:
        redis_client.ping()
        print("✅ Redis connected successfully")
    else:
        print("⚠️ TEST_MODE: skipping Redis ping/connect check")
...
async def fetch_candles_with_cache(session, symbol, timeframe):
    ...
    try:
        if redis_client and not TEST_MODE:
        ...
        if data and redis_client and not TEST_MODE:
            try:
...
def horizontal_levels(closes, highs, lows, lookback=50, binsize=0.002):
    try:
        ...
        if len(closes) >= length:
            pts.extend(closes[-length:])
        if len(highs) >= length:
            pts.extend(highs[-length:])
        if len(lows) >= length:
            pts.extend(lows[-length:])
    except Exception:
        return []
    levels = []
    for p in pts:
...
# ---------------- Pattern detection ----------------
# (unchanged from original but kept for completeness)
        ...
        while i < len(sorted_levels):
            ...
            while j < len(sorted_levels):
                if abs(sorted_levels[j]['price'] - base_price) / base_price < tolerance:
                    ...
                    break
            total_weight = sum(level['strength'] * (level.get('volume') or 1) for level in group)
            weighted_price = sum(level['price'] * level['strength'] * (level.get('volume') or 1) for level in group) / total_weight if total_weight > 0 else base_price
            consolidated.append({
                'price': weighted_price,
                'strength': sum(level['strength'] for level in group),
                'volume': sum(level.get('volume') or 0 for level in group),
                'touch_count': len(group),
                'latest_touch': max(level['timestamp'] for level in group)
            })
            i = j
        consolidated.sort(key=lambda x: x['touch_count'] * x['strength'] * (x['volume'] or 1), reverse=True)
...
def analyze_multi_timeframe_720(candles_4h, candles_1h):
    try:
        ...
        for tf_name, candles in [("4H", candles_4h), ("1H", candles_1h)]:
            ...
            volumes = [c.get('volume', 0) for c in candles[-50:]] if len(candles) >= 1 else []
        ...
        # strength scoring (more sensitive than before)
        strength = 0
        ...
        # signal decision (more reachable trigger and scaled confidence)
        base_conf = int(os.getenv("BASE_CONF", "60"))
        if strength >= SIGNAL_STRENGTH_TRIGGER and nearest_support and nearest_resistance:
            entry = current_price
            ...
            if (tp - entry) > 0 and (entry - sl) > 0:
                ...
                if rr >= MIN_RR_RATIO:
                    conf_val = min(95, base_conf + int(strength * 4))
                    analysis['signals'].append({'side': 'BUY', 'entry': entry, 'sl': sl, 'tp': tp, 'rr': rr, 'confidence': conf_val})
        elif strength <= -SIGNAL_STRENGTH_TRIGGER and nearest_support and nearest_resistance:
            entry = current_price
            ...
            if (entry - tp) > 0 and (sl - entry) > 0:
                ...
                if rr >= MIN_RR_RATIO:
                    conf_val = min(95, base_conf + int(abs(strength) * 4))
                    analysis['signals'].append({'side': 'SELL', 'entry': entry, 'sl': sl, 'tp': tp, 'rr': rr, 'confidence': conf_val})

        # attach debug summary
        analysis['debug'] = {
            'strength': strength,
            'support_count': len(all_support),
            'resistance_count': len(all_resistance),
            'nearest_support': nearest_support,
            'nearest_resistance': nearest_resistance
        }
        print(f"[DEBUG] MultiTF -> strength={strength}, supports={len(all_support)}, resistances={len(all_resistance)}")
        return analysis
    except Exception as e:
...
def save_signal_to_redis(symbol, signal_data):
    if TEST_MODE:
        print("TEST_MODE: skipping save_signal_to_redis")
        return f"signal:{symbol}:TEST:{int(datetime.utcnow().timestamp())}"
...
def update_signal_status(signal_id, status, reason=None):
    if TEST_MODE:
        print(f"TEST_MODE: skipping update_signal_status for {signal_id} -> {status}")
        return True
    if not redis_client:
        return False
    try:
...
async def send_telegram_message(session, text, parse_mode="Markdown"):
    if TEST_MODE:
        print("TEST_MODE: Telegram message (skipped):", text[:200])
        return True
...
async def check_trend_reversal(session, signal, candles_1h):
    try:
        ...
        price_change = (recent_avg - early_avg) / early_avg if early_avg else 0
        reversal = False
...
async def analyze_symbol_720_candles(session, symbol):
    try:
        ...
        try:
            print(json.dumps({'symbol': symbol, 'strength': analysis_result.get('strength'), 'signals': analysis_result.get('signals'), 'debug': analysis_result.get('debug', {})}, indent=2, default=str))
        except Exception:
            pass
        if not analysis_result['signals']:
            print(f"📈 {symbol}: No signals generated (Strength: {analysis_result.get('strength', 0)})")
            return
        # proceed with AI confirmation
        ...
        # compute ai boost (conservative)
        ai_boost = 0
        if ai_decision == 'CONFIRM' and ai_conf:
            ai_boost = int(ai_conf / 8)
        elif ai_decision == 'REJECT' and ai_conf:
            ai_boost = -int(ai_conf / 12)
...
async def main():
    ...
    async with aiohttp.ClientSession() as session:
        if TELEGRAM_BOT_TOKEN and TELEGRAM_CHAT_ID and not TEST_MODE:
